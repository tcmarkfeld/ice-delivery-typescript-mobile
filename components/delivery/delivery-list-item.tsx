...
import { getDateKeyFromIso, toCount } from '@/features/deliveries/delivery-utils';
...
const openMaps = async (address: string): Promise<void> => {
  ...
  await Linking.openURL(`https://maps.google.com/?daddr=${destination}`);
};
...
export const DeliveryListItem = ({
  ...
  const hasSpecialInstructions =
    delivery.special_instructions.length > 0 && delivery.special_instructions.toLowerCase() !== 'none';
  const addons: DeliveryAddon[] = [
    { label: AddonLabel.BagLimes, count: toCount(delivery.bag_limes), iconName: AddonIconName.BagLimes },
    { label: AddonLabel.BagLemons, count: toCount(delivery.bag_lemons), iconName: AddonIconName.BagLemons },
    { label: AddonLabel.BagOranges, count: toCount(delivery.bag_oranges), iconName: AddonIconName.BagOranges },
    {
      label: AddonLabel.MargaritaSalt,
      count: toCount(delivery.marg_salt),
      iconName: AddonIconName.MargaritaSalt,
    },
    { label: AddonLabel.FreezePops, count: toCount(delivery.freeze_pops), iconName: AddonIconName.FreezePops },
    ...
    <View
      ...
      <View style={styles.deliveryHeader}>
        <Text style={styles.deliveryDateText}>
          {formatDate(delivery.start_date) + ' to ' + formatDate(delivery.end_date)}
        </Text>
        <View style={styles.headerBadges}>
          {isPickup ? (
            <View style={styles.pickupBadge}>
              <MaterialCommunityIcons color="#be123c" name="truck-check-outline" size={13} />
          ...
          {addons.map((addon) => (
            <View
              ...
              style={[
                ...
                { backgroundColor: addonPaletteByLabel[addon.label].backgroundColor },
              ...
              <MaterialCommunityIcons
                ...
                size={15}
              />
              <Text style={[styles.deliveryAddonsText, { color: addonPaletteByLabel[addon.label].textColor }]}>
      ...
      {hasSpecialInstructions ? (
        <Text style={styles.deliveryInstructions}>Note: {delivery.special_instructions}</Text>
      ) : null}

      <View style={styles.deliveryActions}>
        <Pressable onPress={() => openMaps(delivery.delivery_address)} style={styles.deliveryActionButton}>
          <MaterialCommunityIcons color="#1f6aa5" name="map-marker-radius" size={16} />
          <Text style={styles.deliveryActionText}>Map</Text>
        </Pressable>
        <Pressable onPress={() => callCustomer(delivery.customer_phone)} style={styles.deliveryActionButton}>
          <MaterialCommunityIcons color="#1f6aa5" name="phone" size={16} />
          <Text style={styles.deliveryActionText}>Call</Text>
        </Pressable>
        <Pressable
          onPress={() => textCustomer(delivery.customer_name, delivery.customer_phone)}
          style={styles.deliveryActionButton}>
          <MaterialCommunityIcons color="#1f6aa5" name="message-text" size={16} />
          <Text style={styles.deliveryActionText}>Text</Text>
        </Pressable>
        {showEditAction ? (
          <Pressable onPress={onPressEdit} style={styles.deliveryActionButton}>
            <MaterialCommunityIcons color="#1f6aa5" name="square-edit-outline" size={16} />
            <Text style={styles.deliveryActionText}>Edit</Text>
          </Pressable>
        ) : null}
        {showDeleteAction ? (
          <Pressable onPress={onPressDelete} style={styles.deliveryActionButton}>
            <MaterialCommunityIcons color="#b91c1c" name="trash-can-outline" size={16} />
