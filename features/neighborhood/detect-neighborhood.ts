...
export const detectNeighborhoodFromAddress = (
  ...
  if (crownPoint.includes(normalizedAddress))
    return neighborhoodByValue.get(15);
  ...
  if (buckIsland.includes(normalizedAddress))
    return neighborhoodByValue.get(16);
  if (oceanHill.includes(normalizedAddress)) return neighborhoodByValue.get(1);
  if (corollaLight.includes(normalizedAddress))
    return neighborhoodByValue.get(2);
  ...
  if (whaleheadRight.includes(normalizedAddress))
    return neighborhoodByValue.get(18);
  if (monterayShores.includes(normalizedAddress))
    return neighborhoodByValue.get(17);
  if (currituckClub.includes(normalizedAddress))
    return neighborhoodByValue.get(4);
